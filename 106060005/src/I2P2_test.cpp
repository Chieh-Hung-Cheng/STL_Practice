  ...
  bool equal ( InputIterator1 first1, InputIterator1 last1, InputIterator2 first2, InputIterator2 last2)
  {
    ...
    while (first1!=last1 || first2!=last2) {
      if(first1!=last1 && first2!=last2){
        if (!(*first1 == *first2)){
          std::cout<<"(std:"<<*first1<<",my:"<<*first2<<")";
        
          answer=false;
        }
        else std::cout<<*first1<<" ";
        ++first1; ++first2;
      }
      else if(first1==last1 && first2!=last2){
        std::cout<<"(std:"<<"N"<<",my:"<<*first2<<")";
          answer=false;
          ++first2;
      }
      else if(first1!=last1 && first2==last2){
        std::cout<<"(std:"<<*first1<<",my:"<<"N"<<")";
          answer=false;
          ++first1;
      }
    }
    std::cout<<std::endl;
    if(!answer)system("read -p 'Error occur, press enter to continue' var");
    return answer;
  }
}
...
const unsigned long op_test_cnt(1 << range);
...
struct empty_and_size {
  ...
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"empty and size"<<std::endl;
...
struct equal {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"equal"<<std::endl;
    if(printall)return check(myequal::equal(s.begin(), s.end(), c.begin(), c.end()), "content of container is not equal");
    else return check(std::equal(c.begin(), c.end(), s.begin()), "content of container is not equal");
...
struct back_and_front {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"back and front"<<std::endl;
...
struct capacity {
  ...
  std::size_t operator()(T &c) const { 
    if(printall)std::cout<<"capacity"<<std::endl;
    return capacity_(c, typename std::is_same<WithCapacity, yes_tag>::type()); 
    }
...
struct copy_assignment_operator {
  ...
  bool operator()(T &clhs, T &crhs, Std &slhs, Std &srhs) const {
    if(printall)std::cout<<"copy const"<<std::endl;
    clhs = crhs;
    slhs = srhs;
...
struct count {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"count key"<<std::endl;
    const auto key(gen());
...
struct clear {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"clear"<<std::endl;
    c.clear();
    s.clear();
    return equal()(c, s);
  }
...
struct erase {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if (c.size()) {
      ...
      if(printall)std::cout<<"erase pos:"<<pos<<std::endl;
...
struct erase_range {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if(printall)std::cout<<"erase range begin(val):"<<*begin_begin<<"end(val):"<<*begin_end<<std::endl;
...
struct find {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"find"<<std::endl;
    const auto key(gen()); 
...
struct insert {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if(printall)std::cout<<"insert pos:"<<pos<<" cnt:"<<cnt<<" val:"<<val<<std::endl;
...
struct insert_key_value {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if(printall)std::cout<<"insert key:"<<key<<" val:"<<val<<std::endl;
...
struct insert_range {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if(printall)std::cout<<"insert range pos:"<<pos<<" cnt:"<<cnt<<std::endl;//???//bug here!!!
...
struct pop_back {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if (c.size()) {
      ...
      if(printall)std::cout<<"pop back"<<std::endl;
...
struct pop_front {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"pop front"<<std::endl;
    bool result(true);
    if (c.size()) {
      iterator_valid_check<decltype(c.begin()), decltype(std::addressof(*c.begin()))> iter_check;
...
struct push_back {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"push back"<<std::endl;
    bool result(true);
    const auto val(gen());
    const auto old_size(c.size());
    const auto old_capacity(capacity<WithCapacity>()(c));
    ...
    if(printall)std::cout<<"push back val:"<<val<<std::endl;
...
struct push_front {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if(printall)std::cout<<"push front val:"<<val<<std::endl;
...
struct random_access {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if (c.size()) {
      ...
      if(printall)std::cout<<"random access pos:"<<pos<<std::endl;
...
struct random_algo {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"random algo"<<std::endl;
...
struct reserve {
  ...
  bool operator()(T &c, Std &s) const {
    ...
    if(printall)std::cout<<"reserve newcap:"<<new_capacity<<std::endl;
...
struct shrink_to_fit {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"shink to fit"<<std::endl;
    c.shrink_to_fit();
    return equal()(c, s);
  }
};
...
struct upper_bound {
  template <class T, class Std>
  bool operator()(T &c, Std &s) const {
    if(printall)std::cout<<"upperbound"<<std::endl;
    const auto key(gen());  
    return check(distance<RandomIter>()(c.begin(), c.lower_bound(key)) == std::distance(s.begin(), s.lower_bound(key)),
                 "lower_bound is not equal");
  }
...
struct iterator_iterate {
  ...
  bool operator()(T &&c) const {
    //if(printall)std::cout<<"iterate"<<std::endl;
    bool result(true);
...
struct iterator_random {
  template <class T>
  bool operator()(T &&c) const {
    //if(printall)std::cout<<"iterator random"<<std::endl;
    bool result(true);
...
void check_list() {
  ...
  for (unsigned long i(0); i != op_test_cnt; ++i) {
    if(showprocess){
      std::cout <<int((float(i)/float(op_test_cnt-1)) * 100.0) <<"% : "<< i<<"/"<<op_test_cnt-1<< "\r";
      std::cout.flush();
    }
  ...
  if(showprocess)std::cout<<std::endl;
}
